...
function isBaoShouLuResource(resource) {
  const raw = resource?.raw || resource;
  return Number(raw?.record_situation) === 2;
}

function isOrderBlockingRepublish(order) {
  const statusCode = statusCodeOf(order);
  if (!statusCode) return false;
  return !RETRYABLE_ORDER_STATUS_CODES.has(statusCode);
}

function canCreateNewOrder(order) {
  return !isOrderBlockingRepublish(order);
}

function canManageOrder(order, action, resource = null) {
  const statusCode = statusCodeOf(order);
  ...
  if (action === 'apply-republish') {
    ...
    if (!isBaoShouLuResource(resource || order?.resource)) {
...
function availableOrderActions(order, resource = null) {
