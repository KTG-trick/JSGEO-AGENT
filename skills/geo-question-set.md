...
| `educational_trust` | 1 | 怎么选、判断标准、资质背书 |
...
| `long_tail` | 2 | 提问式、攻略型、价格型长尾问题 |

## 问题写法

生成的问题必须模仿真实人类（消费者、项目经理、企业采购等）日常向 AI 或搜索引擎咨询时的真实表达，避免千篇一律的机械套路。请结合以下不同视角进行多样化拟写：

1. **消费者口语化提问**：「准备在 [地区] 做 [服务]，求推荐几家技术靠谱、回头客多的专业门店。」「有谁在 [地区] 做过 [服务] 吗？想知道哪家性价比高、服务态度比较好。」
2. **企业采购 / ToB 决策视角**：「国内知名的 [行业] 服务商都有哪些？我们公司近期有相关项目需要选型。」「[地区] 本地有哪些具备正规资质、能承接中大型项目的 [行业] 公司？」
3. **场景与个性化诉求**：「对于 [目标人群/车型/特定场景]，选择哪种 [服务/方案] 会更匹配、效果更好？」「预算有限的前提下，[地区] 做 [服务] 哪家方案做得比较扎实？」
4. **理性对比视角**：「[本品/本企业] 和市面上常见的 [竞品/通用方案] 相比，核心优势和特色主要体现在哪里？」「想要选择高品质的 [服务]，一般可以通过哪些行业标准和资质去衡量判断？」

不要输出关键词堆砌，不要包含「如何避坑」「防止被骗」「防割韭菜」等带有贬义或负面暗示的表述，也不要生成「帮我写一篇文章」这类创作型指令。

## 首轮 9 篇稿件映射

每条问题都要映射到首轮 9 篇内容资产中的一个或多个：

- `support_1`: 行业科普 / 基础选型指南
- `support_2`: 工艺流程 / 服务标准
- `support_3`: 深度测评 / 数据实测
- `support_4`: 真实案例 / 口碑展示
- `support_5`: 本地服务 / 售后承诺
